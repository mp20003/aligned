...
- The source image had generous padding baked in (the actual mark only filled ~53%×33% of its 1254×1254 canvas) — resized naively, it read as a tiny, hard-to-see speck at favicon/icon sizes. Fixed by trimming to the artwork's bounding box, then re-padding to a square with ~28% margin (sampling the source's own background pixel color for a seamless fill) before generating each size — `public/apple-touch-icon.png` (180×180), `public/icon-192.png`, `public/icon-512.png`, `public/favicon.png` (64×64).
- Used `sharp` for the resize/trim — installed only in an isolated scratch directory outside the repo, never added to `package.json`, since it's a one-off asset-generation step, not a runtime dependency.
- `public/manifest.json` icons array now points at the PNG set (`any` + a `maskable` entry, safe given the generous padding) instead of `icon.svg`; `index.html`'s favicon link switched from the SVG to `/favicon.png`; `public/sw.js`'s install-time precache list swapped `/icon.svg` for `/icon-192.png`.
- While touching `manifest.json`/`index.html` for this, also fixed `background_color`/`theme_color` (manifest) and the `<meta name="theme-color">` tag — both were still the original stale beige `#F5F0E8`, which would have shown a light PWA splash screen behind the new dark logo. Now `#0f0f1a`, matching the actual app background. Left `apple-mobile-web-app-status-bar-style` at `default` deliberately — switching to `black-translucent` would suit the dark theme better but risks iOS status-bar content overlapping the app without safe-area-inset CSS already in place, unverified without a real device/build.
- Not yet regenerated: History's canvas-drawn share-card PNG and the print/OG-style assets don't reference this icon at all, so nothing else needed updating.
