...
- No pattern-insight feature — the original History spec called for "one insight sentence generated from the pattern (e.g. 'You skip Physical on weekends')." Never built. The weekly check-in (above) now shows a one-week-back callback, but there's still no aggregate view of check-in answers over time or any computed pattern from `days`. Highest-value next candidate once the current punch list is done.
