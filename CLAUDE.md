...
- No aggregate view of weekly check-in answers over time — the check-in (above) shows a one-week-back callback, but nothing looks across all past answers (e.g. "you've said Physical felt hardest 3 of the last 4 weeks"). Note this is different from History's `generateInsight()`, which already exists and already covers pattern-from-`days` — see the correction note near the top of this doc.
